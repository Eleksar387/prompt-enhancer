// ─────────────────────────────────────────────────────────────────────────────
// LEGACY. This was `src/db.js` — history + caption cache in browser IndexedDB.
// The origin-scoped store was silently wiped twice (see the plan / CLAUDE.md), so
// persistence moved to the local sidecar (`src/db.js` → `/api/…`, `server/`).
//
// This file is NOT imported by the app any more. It is kept as:
//   1. a readable reference for the old schema, and
//   2. the read side of the one-shot IndexedDB → sidecar migration
//      (`migrateFromIndexedDB` in the new `src/db.js` imports `getAllHistory` +
//      `getAllCaptions` from here).
// The old IndexedDB is left intact by the migration as a cold backup.
// ─────────────────────────────────────────────────────────────────────────────
...
// Migration read: every { key, caption } record in the L2 caption store.
export async function getAllCaptions() {
  const db = await openDB()
  return new Promise((resolve, reject) => {
    const req = db.transaction(CAPTION_STORE, 'readonly').objectStore(CAPTION_STORE).getAll()
    req.onsuccess = (e) => resolve(e.target.result || [])
    req.onerror = (e) => reject(e.target.error)
  })
}
