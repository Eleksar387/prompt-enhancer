...
`App.jsx`: the `runWriter` snapshot now stores `caption: frameDescription || null`; `restore()` rehydrates it. A history card shows a `⇄ Adapt` button whenever the entry has `h.caption` **or stored image bytes** (`entryHasStoredImages(h)`). `startAdapt(h)` sets `adaptSourceOverride` immediately (cleared by `switchTarget` / `enhance` / `restore`); if the entry has no stored `caption` it flags `captioning: true` and calls `captionForEntry(h)` — which re-runs the vision step from the entry's stored images via `captionImages(src)` (an optional `src` object overrides the live workspace state) keyed on `h.vision` so recent entries hit the persistent caption cache for free. `cachedVision` takes an optional `model` param for this. `AdaptPanel` disables Generate and shows "👁 Reading the images…" while `captioning`. `LTX_INTRO` / `LTX_GUIDE_INTRO` gained a two-sentence "if NO frame description is given (pure text request)" clause (also fixes the pre-existing bare text-only LTX path).
